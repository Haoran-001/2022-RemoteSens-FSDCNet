...
class ModelNetTrainer(object):
    ...
    def train(self, n_epochs):
        ...
        for epoch in range(n_epochs):
            ...
            for i, data in enumerate(self.train_loader):
                ...
                log_str = 'epoch %d, step %d: train_loss %.3f; train_acc %.3f; lr %.6f' % (epoch + 1, i + 1, loss, acc, self.scheduler.get_lr()[0])
                if (i + 1) % 10 == 0:
                    print(log_str)
                i_acc += i
            # self.optimizer.step()
    ...
    def update_validation_accuracy(self, epoch):
        ...
        # wrong_class = np.zeros(26)
        samples_class = np.zeros(40)
        # samples_class = np.zeros(26)
        all_loss = 0
